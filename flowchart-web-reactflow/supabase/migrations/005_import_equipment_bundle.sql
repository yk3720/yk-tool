-- ADR-014: editor bulk import — equipment hierarchy + flow_documents (1 transaction)
-- Depends on: 003_db2_schema.sql, 004_flow_documents_module_fk.sql
...
begin
  ...
  insert into public.devices (internal_code, display_name, sort_order)
  values (
    v_code,
    v_name,
  ...
  loop
    insert into public.units (device_id, label, sort_order)
    values (v_device_id, v_unit.label, coalesce(v_unit.sort_order, 0))
...
comment on function public.import_equipment_bundle(jsonb) is
  'Editor bulk import: equipment_codes → devices → units → modules → flow_documents. '
  'Payload flows[].payload must be ModuleSnapshot json (prepared by app). Single transaction.';

grant execute on function public.import_equipment_bundle(jsonb) to authenticated;
