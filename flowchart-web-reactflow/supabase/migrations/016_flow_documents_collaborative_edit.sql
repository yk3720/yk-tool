-- 運用方針（2026-06-15）: フロー中身は editor 同士で編集可 · 削除系のみ所有者/admin 制限
-- Reverts 015 flow_documents UPDATE RLS and import flow skip-on-other-owner.
-- 削除の正本: rpc_delete_module / rpc_reset_flow_content / 装置・ユニット削除 RPC（変更なし）
-- Depends on: 015_flow_documents_owner_rls.sql（適用済みなら本ファイルで上書き）
...
comment on policy "flow_documents_insert_editor" on public.flow_documents is
  'Editor or admin may insert flow rows (cloud save · collaborative edit).';
comment on policy "flow_documents_update_editor" on public.flow_documents is
  'Editor or admin may update any flow row (collaborative edit). Delete/reset use RPC owner checks.';

-- ---------------------------------------------------------------------------
-- import_equipment_bundle — restore full flow upsert on re-import (012 behaviour)
-- ---------------------------------------------------------------------------
...
comment on function public.import_equipment_bundle(jsonb) is
  'Editor/admin bulk import. Flow upsert on re-import (collaborative edit). '
  'Module/device delete remains owner-gated via separate RPCs.';
