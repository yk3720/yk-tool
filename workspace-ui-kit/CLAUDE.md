...
- **正本:** `c:/yk-tool/workspace-ui-kit/`（`yk-tool` リポジトリに含める。push は `yk-tool` の remote のみ）
- **講座用 ADS Gitea**（`git.ai-driven-school-portal.com/ADS/workspace-ui-kit`）とは切り離し済み（2026-05-23）
- 受講生向けに再配布する場合は、ADS 側リポへの別途コピー／fork を検討（自動同期なし）
- `.distignore` の想定は従来どおり（`openspec/` `AGENTS.md` は配布対象外の例）
